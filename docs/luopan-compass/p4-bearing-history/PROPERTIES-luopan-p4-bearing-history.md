...
| SE PROPERTIES-v2 F-07 | PROP-SENSOR-037 `buildJsonFromSensors()` `List<Sensor>` parameter impossible in JVM test | PROP-SENSOR-037 | Already resolved in v0.2 (`SensorInfo` wrapper introduced; function signature changed to `List<SensorInfo>`). Confirmed no regression in v0.3. |
| SE PROPERTIES-v2 F-08 | PROP-HIST-042 reflection-based Snackbar duration verification obfuscation-fragile and blocked on Android 14+ | PROP-HIST-042 | Already resolved in v0.2 (reflection removed; behavioral timing assertion used). Confirmed no regression in v0.3. |
