...
| PROP-HIST-064 | The `updateListState()` helper must produce the correct `View.VISIBLE` / `View.GONE` assignments for all four state combinations: (A) 0 records + empty query, (B) 0 records + non-empty query, (C) ≥1 records + any query with matches, (D) ≥1 records + non-empty query with zero matches. | Functional | E2E | `BearingHistoryFragmentTest.kt` (E-2a) | **Reclassified to E2E (SE PROPERTIES-v2 F-06):** `updateListState()` is specified in TSPEC §7.1 as a private Fragment method that directly references Fragment View objects (`searchBar`, `recyclerView`, `emptyNoRecords`, `emptyNoResults`). It cannot be called from a JVM ViewModel test. The four-branch pass condition is verified by seeding Room with the appropriate records, issuing the appropriate search query (or none) via `onView()`, and asserting the resulting visibility of each view: (A) empty-state-no-records VISIBLE + RecyclerView GONE + search bar GONE; (B) empty-state-no-results VISIBLE + RecyclerView GONE + search bar VISIBLE; (C) RecyclerView VISIBLE + both empty-state views GONE; (D) empty-state-no-results VISIBLE + RecyclerView GONE + search bar VISIBLE. All four branches must be covered in the instrumented test. |
...
| PROP-HIST-064 | `updateListState()` four-branch visibility test reclassified from Unit to E2E because the method is a private Fragment helper referencing View objects — not callable from a JVM ViewModel test. |
