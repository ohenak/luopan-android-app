...
| **Cross-reviews addressed** | PM PLAN-v2 (F-01: Snackbar text); TE PLAN-v2 (F-01: B-2a forward ref; F-02: TSPEC snippet conflict; F-03: AT-CAL-03-B2 sequencing bug); SE PROPERTIES-v1 (F-01: espresso-contrib dep; F-02: runTest for Flow DAOs; F-04: TestScope for debounce; F-06: recording adapter; F-08: Robolectric for PROP-CAL-038; F-10: SensorInfo wrapper); PM PROPERTIES-v1 (F-01: FSPEC-HIST-03 update note; F-02: user-observable Snackbar duration; F-03: ms-precision boundary; F-04: E2 integration property; F-05: TSPEC §6.2 anchor for two-launcher); SE PROPERTIES-v2 (F-01: IDriftDetector PLAN task B-3 note + TSPEC cross-ref; F-02: FakeBearingDao hand-written interface + TestScope/setMain; F-03: PROP-CAL-040 reclassified to Integration instrumented; F-04: PROP-HIST-022 source-inspection clause already removed in v0.2 — confirmed; F-05: :benchmark module existence confirmed + referenced in PROP-HIST-071; F-06: PROP-HIST-064 reclassified to E2E; F-07: PROP-SENSOR-037 SensorInfo already clear in v0.2 — confirmed; F-08: PROP-HIST-042 reflection already removed in v0.2 — confirmed); SE PROPERTIES-v3 (F-01: TSPEC §5.5 update prerequisite note added to PROP-SENSOR-037 — se-author must update TSPEC before Phase C C-1; F-02: PROP-HIST-072 source-inspection clause removed + RecordingBearingAdapter assertion added; F-03: PROP-CAL-019b injection mechanism specified) |
...
> **FakeBearingDao implementation note (SE PROPERTIES-v2 F-02):** `BearingHistoryViewModel` is specified in TSPEC §6.3 to accept a `BearingDao` directly (not a `BearingRepository`). Room `@Dao` annotated types are **interfaces** (not abstract classes), so `FakeBearingDao` must be a hand-written class that `implements BearingDao` — this compiles correctly since Room DAOs are interfaces. Do **not** attempt to subclass the Room-generated implementation. All `BearingHistoryViewModelTest.kt` tests must use `Dispatchers.setMain(testDispatcher)` in `@Before` and `Dispatchers.resetMain()` in `@After` (or pass a `CoroutineDispatcher` / `TestScope` via constructor injection) so that `viewModelScope` uses virtual time and `debounce` operators are controllable.
...
| PROP-HIST-064 | The `computeListState()` pure function must produce the correct visibility-state assignments for all four state combinations: (A) 0 records + empty query, (B) 0 records + non-empty query, (C) ≥1 records + any query, (D) same as C but search active. | Functional | Unit | `BearingHistoryViewModelTest.kt` (E-2a) | **Reclassified back to Unit (TE CR-IMPL F-01 resolution):** The implementation exposes `BearingHistoryViewModel.computeListState()` as an `internal` companion function returning a `ListState` data class — a pure JVM-testable function with no Android View dependencies. `updateListState()` in the Fragment delegates to `computeListState()` for all decision logic. The four unit branches are covered in `BearingHistoryViewModelTest.kt`: (A) `updateListState branch A` — `searchBarVisible=false`, `recyclerViewVisible=false`, `emptyNoRecordsVisible=true`, `emptyNoResultsVisible=false`; (B) `updateListState branch B` — `searchBarVisible=true`, `recyclerViewVisible=false`, `emptyNoRecordsVisible=false`, `emptyNoResultsVisible=true`; (C) `updateListState branch C` — `searchBarVisible=true`, `recyclerViewVisible=true`, both empty views false; (D) `updateListState branch D` — same as C with active search query. All four branches pass. The Fragment's View-binding wiring of this state remains implicitly covered by AT-HIST-04-A (State A) in `BearingHistoryFragmentTest.kt` (E2E, Hilt-deferred to Phase 5). |
...
| PROP-HIST-071 | Frame time during fling scroll with 500 records loaded must not exceed 16 ms (60 fps) as measured by `FrameTimingMetric` in the macrobenchmark build variant. | Performance | E2E | `BearingHistoryBenchmark.kt` | **Module confirmed (SE PROPERTIES-v2 F-05):** The `:benchmark` Gradle module exists at `benchmark/build.gradle.kts` with `testInstrumentationRunner = "androidx.benchmark.junit4.AndroidBenchmarkRunner"` and a `benchmark` build type. `BearingHistoryBenchmark.kt` must be placed in `:benchmark`'s `src/benchmark/kotlin/` source set. Macrobenchmark `FrameTimingMetric` p99 ≤ 16 ms; result reported as CI tracked metric (non-blocking gate). |
| PROP-HIST-072 | The `BearingAdapter` must use `ListAdapter<BearingRecord, ViewHolder>` with `DiffUtil.ItemCallback` rather than `notifyDataSetChanged()` to support efficient incremental updates. | Contract | Unit | `BearingAdapterFormatTest.kt` | **SE PROPERTIES-v3 F-02:** Source-inspection assertion removed — a runtime unit test cannot verify the absence of a source call. Use a `RecordingBearingAdapter` subclass (same test double as PROP-HIST-022) to assert at runtime. Pass condition: (1) structural assertion: `adapter.javaClass.superclass` is `ListAdapter::class.java` (or `adapter is ListAdapter`); (2) runtime assertion: after calling `adapter.submitList(listOf(record1, record2))`, `recordingAdapter.notifyDataSetChangedCount == 0`. Note: `RecordingBearingAdapter` must subclass `BearingAdapter`, which extends `ListAdapter` — if `ListAdapter` requires the Android runtime, `BearingAdapterFormatTest.kt` must be annotated with `@RunWith(RobolectricTestRunner::class)` (consistent with `BearingDaoTest.kt`'s project-level Robolectric convention). **Implementation note (TE implementation review F-07):** The current codebase uses a delegation pattern — `RecordingBearingAdapter` holds a `BearingAdapter` delegate and passes callback lambdas to `toggleExpanded()`. This was a deliberate design decision — `BearingAdapter.toggleExpanded()` accepts `onNotifyItemChanged` and `onNotifyDataSetChanged` callback parameters. The delegation approach is equivalent and is accepted as the canonical implementation. |
...
| PROP-CAL-019b | When `DriftDetector` does not emit `TRIGGERED` because `interferenceState = WARNING` violates the COUNTING precondition (REQ Scenario E2 — REQ-CAL-05 Condition B precondition 2), `driftBannerState` must remain `HIDDEN` and the drift banner view must be `GONE` in `BearingHistoryFragment`. | Integration | Integration | `RecalibrationBannerTest.kt` (Scenario E2) | **Injection mechanism specified (SE PROPERTIES-v3 F-03):** `interferenceState` inside `CompassViewModel` is computed by the live `InterferenceDetector` from sensor frames — it cannot be set directly on the ViewModel. The correct test strategy is to inject `FakeDriftDetector(nextEvent = null)` (no `TRIGGERED` event emitted), which accurately simulates real `DriftDetector` behavior when `interferenceState = WARNING` blocks the COUNTING precondition. The ViewModel-level behavior that `TRIGGERED` → banner visible is already tested in PROP-CAL-018/019; the DriftDetector-level behavior that WARNING blocks COUNTING is tested in PROP-CAL-031. This property tests the user-observable UI outcome. **Instrumented test setup:** Launch `CompassActivity` with a custom `ViewModelProvider.Factory` that injects `FakeDriftDetector` configured with `nextEvent = null`. Navigate to the History tab. Assert: drift banner root view (`R.id.banner_drift_root`) has `visibility == GONE`. This assertion is deterministic regardless of the live sensor environment because `FakeDriftDetector` never emits `TRIGGERED`. The real `InterferenceDetector` is left running — if it happens to emit `WARNING` during the test, the interference indicator will be `VISIBLE`, which can be asserted as an additional check; otherwise that second assertion is best-effort. The primary assertion (drift banner `GONE`) is unconditional and must always pass. |
...
| PROP-CAL-036 | The `IDriftDetector` interface must declare `onFrame(accVariance, measuredMagnitudeUt, interferenceState, expectedFieldUt): DriftEvent?` and `reset()`. `DriftEvent` enum must contain exactly the constants `TRIGGERED` and `RESET`. | Contract | Unit | `IDriftDetectorContractTest.kt` (B-2a) | **Creation prerequisite (SE PROPERTIES-v2 F-01):** `IDriftDetector` does not exist in the current codebase. The PLAN mandates its creation in task B-3 (before B-4, which wires it into `CompassViewModel`). TSPEC §5.3 and §9.3b describe `DriftDetector` as a concrete class with no declared interface — this is a known TSPEC gap superseded by the PLAN B-3 mandate and PROP-CAL-037. The `IDriftDetectorContractTest.kt` test file can only be written and pass **after** PLAN task B-3 is complete. Pass condition: interface compiles; both enum constants (`TRIGGERED`, `RESET`) accessible; a minimal `FakeDriftDetector : IDriftDetector` stub compiles (full `FakeDriftDetector` implementation is asserted in PROP-CAL-037 at B-4). |
| PROP-CAL-037 | `CompassViewModel` must accept `IDriftDetector` (not `DriftDetector`) as a constructor parameter so that `FakeDriftDetector` can be injected in unit tests. | Contract | Unit | `CompassViewModelDriftTest.kt` | **Interface dependency (SE PROPERTIES-v2 F-01):** This property is implementable only after PLAN task B-3 (create `IDriftDetector`) and B-4 (update `CompassViewModel` constructor). TSPEC §9.3b shows `FakeDriftDetector` subclassing the concrete `DriftDetector` — that snippet is superseded by the PLAN B-4 mandate. Post-B-4 pass condition: `CompassViewModel` instantiated with `FakeDriftDetector : IDriftDetector` does not throw; sensor loop calls `fake.onFrame()` on each sensor frame. |
| PROP-CAL-038 | The end-to-end wiring `AccelerometerVarianceTracker → DriftDetector.onFrame() → CompassViewModel → driftBannerState` must produce `DriftBannerState.VISIBLE` after 61 seconds of continuous drift with all preconditions met, using real implementations and `FakeClock`. | Integration | Integration (Robolectric) | `CompassViewModelDriftTest.kt` (AT-CAL-INT-01) | **Test file corrected (TE implementation review F-05):** The actual full-chain Robolectric test `PROP-CAL-038 real DriftDetector and AccelerometerVarianceTracker produce VISIBLE after 61s` is implemented in `CompassViewModelDriftTest.kt`, not `DriftDetectorIntegrationTest.kt`. The test is annotated with `@RunWith(RobolectricTestRunner::class)` and uses `ApplicationProvider.getApplicationContext()` to obtain an `Application` instance. Phase 1 (58 s of frames via `FakeClock.advance()`): `driftBannerState.value == HIDDEN`. Phase 2 (advance past 60 s, then one frame with >10% deviation): `driftBannerState.value == VISIBLE`. |

### 2.5 Cooldown Persistence

| ID | Property | Category | Level | Test File | Pass Condition |
|----|----------|----------|-------|-----------|----------------|
| PROP-CAL-040 | The 10-minute drift cooldown timestamp must be persisted in `SettingsRepository.driftCooldownTimestampMs` as a Unix epoch millisecond value, surviving process death. | Functional | Integration | `RecalibrationBannerTest.kt` (AT-CAL-02-D) | **Reclassified to Integration instrumented (SE PROPERTIES-v2 F-03):** A JVM unit test with `FakeSettingsRepository` (an in-memory object) cannot simulate process death — the fake is reset within the same JVM process, which is not equivalent to SharedPreferences being read from on-disk storage after a process restart. This property must be tested in an **instrumented** context using a real `SettingsRepository` backed by a real `SharedPreferences` instance (obtained via `ApplicationProvider.getApplicationContext()` or an `InstrumentationRegistry` context). Pass condition: (1) call `compassViewModel.dismissDriftBanner()` using a real `SettingsRepository`; (2) obtain a second `SettingsRepository` instance backed by the same SharedPreferences file (simulating process restart by re-reading from disk); (3) assert `settings2.driftCooldownTimestampMs > 0L`. The SharedPreferences file must be cleared in `@After` to avoid inter-test contamination. |
...
| PROP-SENSOR-037 | `buildJsonFromSensors()` must be an `internal` pure function (no `Context` or `SensorManager` dependency) to enable JVM unit testing without an Android runtime. Its parameter type must be `List<SensorInfo>`, not `List<android.hardware.Sensor>`. `android.hardware.Sensor` is a `final` class with no public constructor and cannot be instantiated in a JVM unit test. A `SensorInfo` wrapper data class (e.g., `data class SensorInfo(val type: Int, val name: String, val vendor: String, val resolution: Float, val maximumRange: Float, val reportingMode: Int)`) must be introduced; production code that calls `SensorManager.getSensorList()` maps each `Sensor` to a `SensorInfo` before passing to `buildJsonFromSensors()`. | Contract | Unit | `SensorCapabilityLoggerTest.kt` | **TSPEC §5.5 update prerequisite (SE PROPERTIES-v3 F-01):** TSPEC §5.5 currently declares `buildJsonFromSensors(sensors: List<Sensor>, ...)` — this is inconsistent with this property's `List<SensorInfo>` mandate. The TSPEC §5.5 must be corrected by the se-author **before Phase C task C-1 is written**; otherwise the C-1 failing test will use `SensorInfo` but the implementation will be written against `List<Sensor>` per the TSPEC, causing a compile-time type mismatch. Required TSPEC §5.5 corrections: (1) define `data class SensorInfo(val type: Int, val name: String, val vendor: String, val resolution: Float, val maximumRange: Float, val reportingMode: Int)` in the `com.luopan.compass.diagnostics` package; (2) change `buildJsonFromSensors()` parameter to `List<SensorInfo>`; (3) show the mapping `sensors.map { SensorInfo(it.type, it.name, it.vendor, it.resolution, it.maximumRange, it.reportingMode) }` in `buildJson()`. This is a TSPEC correction required before implementation; the PROPERTIES contract (`List<SensorInfo>`) is authoritative. Post-correction pass condition: `buildJsonFromSensors(listOf(SensorInfo(type=1, name="test", vendor="test", resolution=0.1f, maximumRange=50f, reportingMode=0)))` callable in a JVM unit test without Robolectric or instrumentation; returns a non-empty JSON string. |
...
| AT-CAL-02-D | PROP-CAL-040, PROP-CAL-041 | D-1c |
...
| AT-CAL-INT-01 | PROP-CAL-038 | B-7, B-8 | <!-- Test implemented in CompassViewModelDriftTest.kt (not DriftDetectorIntegrationTest.kt) — see PROP-CAL-038 correction --> |
...
| SE PROPERTIES-v2 F-01 | `IDriftDetector` interface does not exist in TSPEC or codebase; PROP-CAL-036/037 are unimplementable until it is created | PROP-CAL-036, PROP-CAL-037 | Notes added to both properties: `IDriftDetector` must be created as PLAN task B-3 (before B-4). TSPEC §5.3/§9.3b shows concrete `DriftDetector` — superseded by the PLAN B-3 mandate. Pass conditions updated to reflect the post-B-3 / post-B-4 implementation sequence. |
| SE PROPERTIES-v2 F-02 | `FakeBearingDao` implementation approach unclear; Room DAO abstract-class concern | PROP-HIST-031, PROP-HIST-032, PROP-HIST-033 | Section note added to §1.4 clarifying that `BearingDao` is a Room `@Dao` interface (not abstract class), so `FakeBearingDao` is a hand-written class that `implements BearingDao`. `Dispatchers.setMain(testDispatcher)` requirement reinforced in pass conditions. |
| SE PROPERTIES-v2 F-03 | PROP-CAL-040 claimed process-death persistence verifiable by a JVM unit test with `FakeSettingsRepository` — structurally impossible | PROP-CAL-040 | Reclassified from Unit to Integration (instrumented). Test file changed from `CompassViewModelDriftTest.kt` to `RecalibrationBannerTest.kt`. Pass condition updated to require a real `SettingsRepository` backed by real SharedPreferences in an instrumented context, simulating process restart by constructing a second `SettingsRepository` instance over the same SharedPreferences file. |
| SE PROPERTIES-v2 F-04 | PROP-HIST-022 pass condition contained both a runtime assertion and a source-code inspection clause | PROP-HIST-022 | Already resolved in v0.2 (source-code inspection clause removed; hand-rolled `RecordingBearingAdapter` approach used). Confirmed no regression in v0.3. |
| SE PROPERTIES-v2 F-05 | PROP-HIST-071 macrobenchmark module existence unconfirmed | PROP-HIST-071 | `:benchmark` module confirmed at `benchmark/build.gradle.kts` with `AndroidBenchmarkRunner` and `benchmark` build type. Pass condition updated to reference the `:benchmark` module and its `src/benchmark/kotlin/` source set. |
| SE PROPERTIES-v2 F-06 | PROP-HIST-064 `updateListState()` is a private Fragment method unreachable from a JVM ViewModel test | PROP-HIST-064 | Reclassified from Unit / `BearingHistoryViewModelTest.kt` to E2E / `BearingHistoryFragmentTest.kt`. Pass condition rewritten to describe four Espresso-based instrumented sub-tests that seed Room and assert view visibility for each state combination. |
| TE CR-IMPL F-01 | PROP-HIST-064 was reclassified to E2E in v0.4 but the implementation exposes `computeListState()` as a JVM-testable companion function; four-branch unit tests exist and pass | PROP-HIST-064 | Reclassified back to Unit. Test file changed back to `BearingHistoryViewModelTest.kt`. Pass condition updated to reference `computeListState()` unit tests covering all four branches (A–D). E2E wiring coverage deferred to Phase 5 Hilt migration alongside AT-HIST-04-A. |
| SE PROPERTIES-v2 F-07 | PROP-SENSOR-037 `buildJsonFromSensors()` `List<Sensor>` parameter impossible in JVM test | PROP-SENSOR-037 | PROPERTIES contract correctly mandates `List<SensorInfo>` since v0.2. However SE PROPERTIES-v3 confirmed that TSPEC §5.5 still declares `List<Sensor>` — the TSPEC was not updated. PROP-SENSOR-037 now carries an explicit prerequisite note (v0.4) requiring the se-author to correct TSPEC §5.5 before Phase C task C-1. The PROPERTIES contract is authoritative; TSPEC §5.5 is the artifact requiring correction. |
| SE PROPERTIES-v2 F-08 | PROP-HIST-042 reflection-based Snackbar duration verification obfuscation-fragile and blocked on Android 14+ | PROP-HIST-042 | Already resolved in v0.2 (reflection removed; behavioral timing assertion used). Confirmed no regression in v0.3 and v0.4. |
| SE PROPERTIES-v3 F-01 | TSPEC §5.5 still declares `buildJsonFromSensors(sensors: List<Sensor>)` contradicting PROP-SENSOR-037's `List<SensorInfo>` mandate; te-author's v0.3 "already confirmed" claim was incorrect | PROP-SENSOR-037 | Prerequisite note added to PROP-SENSOR-037 requiring se-author to correct TSPEC §5.5 (define `SensorInfo` data class; change parameter type; add mapping in `buildJson()`) before Phase C task C-1 begins. The PROPERTIES contract is correct and unchanged; the TSPEC is the artifact needing correction. |
| SE PROPERTIES-v3 F-02 | PROP-HIST-072 retained a source-inspection clause ("no `notifyDataSetChanged()` call in adapter source") — a static assertion not verifiable at runtime | PROP-HIST-072 | Source-inspection clause removed. Pass condition now: (1) structural assertion that adapter `is ListAdapter`; (2) runtime assertion via `RecordingBearingAdapter.submitList()` that `notifyDataSetChangedCount == 0`. Robolectric requirement noted for `ListAdapter` subclassing. |
| SE PROPERTIES-v3 F-03 | PROP-CAL-019b pass condition did not specify how `interferenceState = WARNING` is achieved in the instrumented test; `interferenceState` is computed from the live sensor loop, not directly settable | PROP-CAL-019b | Pass condition rewritten. Injection mechanism: inject `FakeDriftDetector(nextEvent = null)` via custom `ViewModelProvider.Factory`. The fake never emits `TRIGGERED`, correctly simulating real DriftDetector behavior when `interferenceState = WARNING` blocks COUNTING. Primary assertion: drift banner `GONE` — unconditional and deterministic regardless of live sensor state. DriftDetector unit behavior (WARNING blocks COUNTING) tested in PROP-CAL-031; ViewModel TRIGGERED→banner logic tested in PROP-CAL-018/019. |
...
| PROP-HIST-064 | `updateListState()` four-branch visibility test: reclassified Unit→E2E in v0.4 (SE PROPERTIES-v2 F-06) because the method was private. Implementation exposed `computeListState()` as a JVM-testable companion function; reclassified back to Unit in v0.5 (TE CR-IMPL F-01) with all four branches covered in `BearingHistoryViewModelTest.kt`. |
| PROP-CAL-040 | Drift cooldown persistence reclassified from Unit to Integration (instrumented) because `FakeSettingsRepository` cannot simulate process-death/SharedPreferences disk persistence. |
