 ...
 *
 * TSPEC §3.5 — wraps [BearingDao]. Room 2.1+ suspend DAOs already dispatch to Dispatchers.IO
 * internally, so no withContext wrapper is needed here.
...
class BearingRepositoryImpl(private val dao: BearingDao) : BearingRepository {
     ...
     */
    override suspend fun insert(record: BearingRecord) = dao.insert(record)
     ...
     */
    override suspend fun getAll(): List<BearingRecord> = dao.getAll()
